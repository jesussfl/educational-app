...
const WorldSections = ({ sections, completedLessons, customRef, setRef }) => {
  // const [ref, setRef] = useState(null);
  const { currentCoords, sectionCoords, addSectionCoords, sectionHeight, addSectionHeight, reset } = useScrollStore();

  useEffect(() => {
    if (customRef && currentCoords && sectionCoords && sectionHeight) {
      customRef.scrollTo({
        x: 0,
  ...
  }, [currentCoords, sectionCoords, sectionHeight, customRef]);
