	...
	return (
		<ScrollView style={{ padding: 24, gap: 24 }}>
			{isLoading ? (
				...
				data.crefinexWorlds.data.map((world) => <Card key={world.id} name={world.attributes.name} description={world.attributes.description} imgSource={`http://192.168.0.108:1337${world.attributes.image.data.attributes.formats.thumbnail.url}`} mainAction={() => updateCurrentWorld(world.id)} />)
			)}
		</ScrollView>
	);
};
