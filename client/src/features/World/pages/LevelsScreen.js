		...
		<ScrollView style={{ padding: 24, gap: 24 }}>
			{isLoading ? (
				...
				data.crefinexWorlds.data.map((world) => {
					console.log(`${API_URL}/${world.attributes.image.data.attributes.formats.thumbnail.url}`);
					return <Card key={world.id} name={world.attributes.name} description={world.attributes.description} imgSource={`${API_URL}${world.attributes.image.data.attributes.formats.thumbnail.url}`} mainAction={() => updateCurrentWorld(world.id)} />;
				})
			)}
		</ScrollView>
	);
